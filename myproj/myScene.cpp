...
void myScene::readObjectsFromFolder(const std::string& path)
{
    ...
    for (auto& p : fs::directory_iterator(dir))
    {
        if (p.path().extension() == ".obj")
        {
            ...
            obj->readObjects(p.path().string(), false, false);
            obj->createmyVAO();
            addObjects(obj, p.path().filename().replace_extension().string());
        }
    }
}
